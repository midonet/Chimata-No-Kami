...
sshconfig: preflight autogenerated
	...
	@find $(shell dirname $(SSHCONFIG))/.config.fragment.*.txt | sort -n | uniq | \
		xargs -n1 --no-run-if-empty cat | \
			tee $(SSHCONFIG)
	@find $(shell dirname $(SSHCONFIG))/.config.fragment.*.txt | \
		xargs -n1 --no-run-if-empty rm
